...
def thumbIncrementCheck(lmList) -> int:
...
def drawTextInDifferentColors(img, text: str, x = 10, y = 70):
    ...
    if text == "NOTHING":
        cv2.putText(img, str(text), (x, y), fontFace=cv2.FONT_HERSHEY_PLAIN, thickness=3, color=(0, 0, 255), fontScale=3)
    else:
        cv2.putText(img, str(text), (x, y), fontFace=cv2.FONT_HERSHEY_PLAIN, thickness=3, color=(0, 255, 0), fontScale=3)
        ...
        if hand_exists:
            ...
            for i in range(0, 5):
                if i == 0:
                    ...
                    # 8: Index finger
                    # 12: Middle finger
                    if (lmlist[tipIDs[i]][2] < lmlist[dipIDs[i]][2]) and (tipIDs[i] in (8, 12)): # if either index or middle
                        count += 1
                        cc += 1
